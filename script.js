...
let dailyTodo = [], weeklyTodo = [], routineTodo = [], birthdays = [], friends = []; 
...
function setCustomTime(prefix, timeStr) {
    ...
    if(!h || !m) return;
    if(timeStr) { 
        let p = timeStr.split(':'); h.value = p[0]; m.value = p[1]; 
    } else { 
        h.value = ""; m.value = ""; 
    }
}
...
function unlockModalFields() {
    ...
    if(nameF) nameF.disabled = false; 
    if(descF) descF.disabled = false;
    if(impF) impF.disabled = false; 
    if(labelF) labelF.innerText = "Date";
        ...
        if (ghosts.length > 0) {
            ...
            if (pastDates.length > 0) {
                pastDates.forEach(pastOcc => { 
                    operations.push(db.collection("tasks").add({ name: t.name, desc: t.desc || "", date: pastOcc.date, time: pastOcc.time, reminders: t.reminders || [], importance: t.importance, completed: true, completedAtStr: pastOcc.date, userId: t.userId, createdAt: t.createdAt || Date.now(), duplicateDays: [] })); 
                });
                if (remainingDates.length > 0) {
                    let nextMain = remainingDates.shift(); 
                    operations.push(db.collection("tasks").doc(t.id).update({ date: nextMain.date, time: nextMain.time, duplicateDays: remainingDates }));
                } else { 
...
function runNotificationEngine() {
    ...
    tasks.forEach(t => {
        ...
        if (displayDate === todayStr && displayTime) {
            ...
            if ((now - taskTimeObj) / 60000 >= 30) { 
                toggleTaskCheck(t.id, false, displayDate); return; 
            }
        }
        ...
        if (displayTime && t.reminders && t.reminders.length > 0) {
            ...
            t.reminders.forEach(r => {
                if (minutesRemaining === Number(r)) {
                    const key = `custom-${t.id}-${displayDate}-${r}`; 
                    let notifs = JSON.parse(localStorage.getItem('listme_sent_notifs')) || {};
                    if (!notifs[key]) { 
    ...
    if ("Notification" in window && Notification.permission === "granted") {
        if (navigator.serviceWorker && navigator.serviceWorker.controller) { 
            navigator.serviceWorker.ready.then(reg => { 
                reg.showNotification(title, { body: body, icon: "https://cdn-icons-png.flaticon.com/512/906/906334.png" }); 
            }); 
        } else { 
...
document.querySelectorAll('.reminder-badge').forEach(badge => { 
    badge.onclick = () => { 
        if(!badge.classList.contains('disabled-frozen')) { badge.classList.toggle('active'); } 
    }; 
});
...
auth.onAuthStateChanged((user) => {
    if (user) {
        ...
        document.getElementById('main-nav').style.display = 'none'; stopRealtimeSync(); 
        document.querySelectorAll('main > section').forEach(s => s.style.display = 'none'); 
...
function autoSaveNickname() { 
    ...
    if (currentUser) { 
        db.collection("users").doc(currentUser.uid).set({ nickname: nick }, { merge: true }).then(() => { 
            userNickname = nick; showToast("Surnom mis à jour ! ✨"); 
        }); 
    } 
}
...
function removeFriend(fUid) { 
    ...
    db.collection("users").doc(currentUser.uid).set({following: friends}, {merge: true}).then(() => { 
        ...
        sharedTasks = sharedTasks.filter(t => t.userId !== fUid); 
        renderCalendar(); showToast("Agenda retiré ! 🗑️"); 
    }); 
}
...
if(btnAddFriend) { 
    btnAddFriend.onclick = () => { 
        ...
        db.collection("users").where("shareCode", "==", code).get().then(snapshot => { 
            ...
            startFriendSync(friendUid, friendName); 
            renderFriendsList(); 
...
function openGhostModal(id) { 
    ...
    ghosts.forEach((g, index) => { 
        ...
        list.innerHTML += `<div style="display:flex; justify-content:space-between; align-items:center; padding:12px; background:rgba(128,128,128,0.08); border-radius:12px;"><span style="font-size: 0.95rem;">📅 <b>${dFr}</b> ${g.time ? `<span style="opacity:0.6; margin:0 5px;">|</span> ⏰ <b>${g.time}</b>` : ''}</span><button onclick="removeGhostDate('${task.id}', ${index})" style="background:none; border:none; color:var(--danger); cursor:pointer; font-size:1.5rem; display:flex; align-items:center; line-height:1; padding:0 5px;">×</button></div>`; 
    }); 
...
function removeGhostDate(taskId, ghostIndex) { 
    ...
    if(task && task.duplicateDays) { 
        ...
        db.collection("tasks").doc(taskId).update({ duplicateDays: ghosts }).then(() => { 
            showToast("Date supprimée ! 🗑️"); openGhostModal(taskId); 
...
function renderTasks() { 
    ...
    if (taskSubView === "active") { 
        ...
        const chronoSort = (a, b) => { let dateA = a.currentDisplayDate || a.date, dateB = b.currentDisplayDate || b.date; if (dateA !== dateB) return dateA.localeCompare(dateB); let timeA = a.currentDisplayTime || "", timeB = b.currentDisplayTime || ""; if (!timeA) return -1; if (!timeB) return 1; return timeA.localeCompare(timeB); }; 
    ...
    if(!currentStatus) { 
        ...
        if (isMultiDate && specificDate) { 
            ...
            if (remainingDates.length > 0) { 
                ...
                let nextMainDate = remainingDates.shift(); db.collection("tasks").doc(id).update({ date: nextMainDate.date || nextMainDate, time: nextMainDate.time || task.time || "", duplicateDays: remainingDates }); 
...
function renderCalendar() { 
    ...
    if (viewState === 'day') { 
        if (btnBday) btnBday.style.display = 'block'; if (legend) legend.style.display = 'flex'; 
    } else { 
        if (btnBday) btnBday.style.display = 'none'; if (legend) legend.style.display = 'none'; 
    } 
        ...
        for (let i = 1; i <= days; i++) { 
            ...
            if (dayColors.length > 0) { 
                ...
                if (dayColors.length === 1) bgStyle = dayColors[0]; 
                    ...
                    let stops = [], step = 100 / dayColors.length; dayColors.forEach((color, idx) => { stops.push(`${color} ${idx * step}%`, `${color} ${(idx+1) * step}%`); }); bgStyle = `linear-gradient(to right, ${stops.join(', ')})`; 
                } 
...
function openCalendarDayModal(day, monthName, year, totalDayTasks, currentFullDate, holiday, dayBirthdays) { 
    ...
    if(totalDayTasks.length === 0 && !holiday && (!dayBirthdays || dayBirthdays.length === 0)) { 
        ...
        totalDayTasks.forEach(t => { 
            ...
            const ownerTag = t.ownerName ? ` <small style="opacity:0.5; font-style:italic; margin-left:5px;">(par ${t.ownerName})</small>` : ''; 
...
function renderTodo() { 
    ...
    if(todoMode === 'daily') { 
        ...
        presidentialOrder.forEach(dayNum => { 
            let combinedTasks = [...weeklyTodo.filter(it => parseInt(it.dayOfWeek) === dayNum), ...routineTodo.filter(it => parseInt(it.dayOfWeek) === dayNum)]; 
...
function openTodoModal(time, isWeeklyOrRoutine, dayNum = 1, isRoutine = false) { 
    editingTodoId = null; setCustomTime('todo-time', time); document.getElementById('todo-task-name').value = ''; 
    ...
    if(isWeeklyOrRoutine) { 
        document.getElementById('todo-day-selector-block').style.display = 'flex'; document.getElementById('todo-day-select').value = dayNum; 
    } else { 
...
function editTodoItem(id, name, time, isWeeklyOrRoutine, dayNum = 1, isRoutine = false) { 
    editingTodoId = id; setCustomTime('todo-time', time); document.getElementById('todo-task-name').value = name; 
...
if (btnSaveTask) { 
    btnSaveTask.onclick = () => { 
        const n = document.getElementById('task-name').value.trim(), dStr = document.getElementById('task-desc').value.trim(), singleDate = document.getElementById('task-date').value, time = getCustomTime('task-time'), imp = document.getElementById('task-importance').value, reminders = getSelectedRemindersFromBadges(); 
        if(n && currentUser) { 
...
if (btnSaveTodo) { 
    btnSaveTodo.onclick = () => { 
        const n = document.getElementById('todo-task-name').value.trim(), t = getCustomTime('todo-time'), isWeekly = document.getElementById('save-todo').getAttribute('data-weekly-mode') === 'true', isRoutine = document.getElementById('save-todo').getAttribute('data-routine-mode') === 'true'; 
        if(n && t && currentUser) { 
...
if (btnLogin) { 
    btnLogin.onclick = () => { const email = document.getElementById('auth-email').value, pass = document.getElementById('auth-pass').value; if(email && pass) auth.signInWithEmailAndPassword(email, pass).then(() => { showToast("Ravi de vous revoir ! 👋"); }).catch(err => showToast("Erreur : " + err.message)); }; 
}

let btnRegister = document.getElementById('btn-register'); 
if (btnRegister) { 
    btnRegister.onclick = () => { const email = document.getElementById('auth-email').value, pass = document.getElementById('auth-pass').value; if(email && pass) auth.createUserWithEmailAndPassword(email, pass).then(() => showToast("Compte créé avec succès ! 🎉")).catch(err => showToast("Erreur : " + err.message)); }; 
}

let btnGoogle = document.getElementById('btn-google'); 
if (btnGoogle) { 
    btnGoogle.onclick = () => { const provider = new firebase.auth.GoogleAuthProvider(); auth.signInWithPopup(provider).then(() => { showToast("Connexion Google réussie ! 🚀"); }).catch((err) => { showToast("Erreur Google : " + err.message); }); }; 
}

let btnLogout = document.getElementById('btn-logout'); 
if (btnLogout) { 
    btnLogout.onclick = () => { auth.signOut().then(() => { showToast("Déconnexion réussie."); }); }; 
}
