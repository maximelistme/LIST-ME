...
let tasks = [], sharedTasks = [], dailyTodo = [], weeklyTodo = [], routineTodo = [], birthdays = [], friends = []; 
...
// --- UTILITAIRES DE TEMPS ET DIVERS ---
...
function unlockModalFields() {
    const nameF = document.getElementById('task-name'), descF = document.getElementById('task-desc');
    const impF = document.getElementById('task-importance'), labelF = document.getElementById('date-input-label');
    if(nameF) nameF.disabled = false; if(descF) descF.disabled = false;
    if(impF) impF.disabled = false; if(labelF) labelF.innerText = "Date";
...
function runNotificationEngine() {
    ...
    let todayMD = todayStr.substring(5), tomorrow = new Date(); tomorrow.setDate(now.getDate() + 1);
    ...
    if (hour === 9 && minute === 0) {
        ...
        todayBirthdays.forEach(b => {
             let key = `bday-j-${b.id}-${todayStr.substring(0,4)}`; let notifs = JSON.parse(localStorage.getItem('listme_sent_notifs')) || {};
             if (!notifs[key]) { sendNotification("🎂 Joyeux Anniversaire !", `C'est l'anniversaire de ${b.name} aujourd'hui !`); notifs[key] = true; localStorage.setItem('listme_sent_notifs', JSON.stringify(notifs)); }
        });
        let tomorrowBirthdays = birthdays.filter(b => b.date.endsWith(tomorrowMD));
        tomorrowBirthdays.forEach(b => {
             let key = `bday-v-${b.id}-${todayStr.substring(0,4)}`; let notifs = JSON.parse(localStorage.getItem('listme_sent_notifs')) || {};
             if (!notifs[key]) { sendNotification("🎁 Bientôt un anniversaire", `C'est l'anniversaire de ${b.name} demain !`); notifs[key] = true; localStorage.setItem('listme_sent_notifs', JSON.stringify(notifs)); }
        });
        if (todayStr.endsWith('-01')) {
             ...
             if (thisMonthBirthdays.length > 0) {
                 let key = `bday-m-${todayStr.substring(0,7)}`; let notifs = JSON.parse(localStorage.getItem('listme_sent_notifs')) || {};
                 if (!notifs[key]) { sendNotification("🗓️ Anniversaires du mois", `Il y a ${thisMonthBirthdays.length} anniversaire(s) prévu(s) ce mois-ci.`); notifs[key] = true; localStorage.setItem('listme_sent_notifs', JSON.stringify(notifs)); }
             }
        }
    }

    if (dayOfWeek === 0 && hour === 18 && minute === 0) {
        const key = `recap-${todayStr}`; let notifs = JSON.parse(localStorage.getItem('listme_sent_notifs')) || {};
        if (!notifs[key]) {
    ...
    tasks.forEach(t => {
        ...
        if (displayDate === todayStr && displayTime) {
            ...
            if ((now - taskTimeObj) / 60000 >= 30) { toggleTaskCheck(t.id, false, displayDate); return; }
        }

        const diffDays = Math.ceil((new Date(displayDate) - now) / (1000 * 60 * 60 * 24));
        if (diffDays === 1 && hour === 20 && minute === 0) {
            const key = `veille-${t.id}-${displayDate}`; let notifs = JSON.parse(localStorage.getItem('listme_sent_notifs')) || {};
            if (!notifs[key]) { sendNotification("⏰ Rappel : C'est pour demain !", `Ne pas oublier : "${t.name}" prévu demain.`); notifs[key] = true; localStorage.setItem('listme_sent_notifs', JSON.stringify(notifs)); }
        }
        ...
        if (displayTime && t.reminders && t.reminders.length > 0) {
            ...
            t.reminders.forEach(r => {
                if (minutesRemaining === Number(r)) {
                    const key = `custom-${t.id}-${displayDate}-${r}`; let notifs = JSON.parse(localStorage.getItem('listme_sent_notifs')) || {};
                    if (!notifs[key]) { sendNotification(`🔔 Rappel : ${t.name}`, `Commence dans ${r} minutes.`); notifs[key] = true; localStorage.setItem('listme_sent_notifs', JSON.stringify(notifs)); }
                }
            });
        }
    });
...
// --- SYNCHRONISATION PRIVÉE ET PARTAGÉE ---
...
function startRealtimeSync(userId) {
    ...
    // Lancement de la synchro des agendas liés
    friends.forEach(f => startFriendSync(f.uid, f.nickname));
}

function startFriendSync(fUid, fName) {
    if(friendUnsubscribes[fUid]) return;
    friendUnsubscribes[fUid] = db.collection("tasks").where("userId", "==", fUid).onSnapshot((snapshot) => {
        sharedTasks = sharedTasks.filter(t => t.userId !== fUid); // Nettoie les anciennes données
        snapshot.forEach((doc) => { 
            let data = doc.data(); data.id = doc.id; data.ownerName = fName; // Marqueur visuel "par Untel"
...
function stopRealtimeSync() { 
    ...
    tasks = []; sharedTasks = []; dailyTodo = []; weeklyTodo = []; routineTodo = []; birthdays = []; friends = [];
}
...
function renderTasks() {
    ...
    filteredList.forEach(t => {
        ...
        const displayDateStr = t.currentDisplayDate || t.date, displayTimeStr = t.currentDisplayTime || t.time || "";
        ...
        if (t.completed) {
            ...
            d.className = `${isMultiDate ? 'task-card stacked-task' : 'task-card'} ${t.importance} ${t.isImminent ? 'is-imminent' : ''}`;
...
// --- RENDER CALENDAR (FUSION DES AGENDAS) ---
...
function renderCalendar() {
    ...
    document.querySelectorAll('#calendar-page .bubble').forEach(b => b.classList.remove('active')); document.getElementById(`btn-${viewState}`).classList.add('active');

    const btnBday = document.getElementById('btn-add-birthday'), legend = document.getElementById('calendar-legend');
    if (viewState === 'day') { if (btnBday) btnBday.style.display = 'block'; if (legend) legend.style.display = 'flex'; } 
    else { if (btnBday) btnBday.style.display = 'none'; if (legend) legend.style.display = 'none'; }

    if (viewState === 'year') {
        c.className = 'grid-years'; t.innerText = "Années";
        for (let i = selectedYear - 4; i <= selectedYear + 4; i++) { const d = document.createElement('div'); d.className = `grid-item ${i === selectedYear ? 'selected' : ''}`; d.innerText = i; d.onclick = () => { selectedYear = i; setViewState('month'); }; c.appendChild(d); }
    } else if (viewState === 'month') {
        c.className = 'grid-months'; t.innerText = selectedYear;
        monthNames.forEach((n, i) => { const d = document.createElement('div'); d.className = `grid-item ${i === selectedMonth ? 'selected' : ''}`; d.innerText = n; d.onclick = () => { selectedMonth = i; setViewState('day'); }; c.appendChild(d); });
    } else {
        c.className = 'calendar-grid'; t.innerText = `${monthNames[selectedMonth]} ${selectedYear}`;
        const days = new Date(selectedYear, selectedMonth + 1, 0).getDate(), yearHolidays = getFrenchHolidays(selectedYear); 
        
        for (let i = 1; i <= days; i++) {
            const ds = `${selectedYear}-${String(selectedMonth + 1).padStart(2, '0')}-${String(i).padStart(2, '0')}`, md = `${String(selectedMonth + 1).padStart(2, '0')}-${String(i).padStart(2, '0')}`; 
            const holiday = yearHolidays.find(h => h.date === ds), dayBirthdays = birthdays.filter(b => b.date.endsWith(md));
            
            // Fusion des tâches perso et partagées pour les pastilles de couleur du calendrier
            const myDayTasks = tasks.filter(tk => { if (tk.date === ds) return true; if (tk.duplicateDays) return tk.duplicateDays.some(g => (typeof g === 'string' ? g : g.date) === ds); return false; });
            const friendDayTasks = sharedTasks.filter(tk => { if (tk.date === ds) return true; if (tk.duplicateDays) return tk.duplicateDays.some(g => (typeof g === 'string' ? g : g.date) === ds); return false; });
            const totalTasks = [...myDayTasks, ...friendDayTasks];

            let dayColors = [];
            if (totalTasks.length > 0) { const imps = totalTasks.map(tk => tk.importance); if(imps.includes('high')) dayColors.push('var(--danger)'); if(imps.includes('medium')) dayColors.push('var(--warning)'); if(imps.includes('low')) dayColors.push('var(--success)'); }
            if (holiday) dayColors.push('var(--holiday-color)'); if (dayBirthdays.length > 0) dayColors.push('var(--birthday-color)');
            ...
            if (dayColors.length > 0) {
                ...
                else { let stops = [], step = 100 / dayColors.length; dayColors.forEach((color, idx) => { stops.push(`${color} ${idx * step}%`, `${color} ${(idx+1) * step}%`); }); bgStyle = `linear-gradient(to right, ${stops.join(', ')})`; }
            ...
            let countHtml = ""; if(totalTasks.length > 0) countHtml = `<span style="position:absolute; top:4px; right:4px; font-size:0.65rem; font-weight:bold; color:var(--primary-dark); background:rgba(0, 206, 209, 0.15); border-radius:10px; padding:2px 5px;">+${totalTasks.length}</span>`;
            
            div.onclick = () => openCalendarDayModal(i, monthNames[selectedMonth], selectedYear, totalTasks, ds, holiday, dayBirthdays);
            div.innerHTML = `${countHtml}<span style="font-size:0.6rem; opacity:0.5; display:block; margin-top: ${totalTasks.length>0 ? '10px' : '4px'};">${dayInitials[new Date(selectedYear, selectedMonth, i).getDay()]}</span><b>${i}</b>${bottomLineHtml}`;
            c.appendChild(div);
        }
    }
}

function openCalendarDayModal(day, monthName, year, totalDayTasks, currentFullDate, holiday, dayBirthdays) {
    ...
    if (dayBirthdays && dayBirthdays.length > 0) { dayBirthdays.forEach(b => { container.innerHTML += `<div style="padding: 12px; border-radius: 12px; border-left: 6px solid var(--birthday-color); background: rgba(232, 67, 147, 0.05); margin-bottom: 5px; display:flex; justify-content:space-between; align-items:center;"><strong style="color:var(--birthday-color);">🎂 Anniv. de ${b.name}</strong></div>`; }); }

    if(totalDayTasks.length === 0 && !holiday && (!dayBirthdays || dayBirthdays.length === 0)) { container.innerHTML = '<p style="text-align:center; opacity:0.5; font-style:italic;">Rien de prévu</p>'; } 
    else {
        totalDayTasks.forEach(t => {
            let specificTime = t.time, isCopy = false;
            if (t.date !== currentFullDate) { isCopy = true; let ghost = t.duplicateDays.find(g => (typeof g === 'string' ? g : g.date) === currentFullDate); if (ghost && ghost.time) specificTime = ghost.time; }
            const isCopyTag = isCopy ? ' <small style="opacity:0.6; font-size:0.75rem;">(Prévu 🗓️)</small>' : '';
            const ownerTag = t.ownerName ? ` <small style="opacity:0.5; font-style:italic;">(par ${t.ownerName})</small>` : '';
            container.innerHTML += `<div style="padding: 12px; border-radius: 12px; border-left: 6px solid var(--${t.importance === 'high'?'danger':t.importance==='medium'?'warning':'success'}); background: rgba(128,128,128,0.05); text-align:left;"><strong style="${t.completed ? 'text-decoration:line-through; opacity:0.5;' : ''}">${t.name}</strong>${ownerTag}${isCopyTag}${specificTime ? `<span style="float:right; font-size:0.85rem; opacity:0.7;">⏰ ${specificTime}</span>`:''}</div>`; 
        });
    }
...
function renderTodo() {
    ...
    if (todoMode === 'daily') document.getElementById('btn-daily').classList.add('active'); else if (todoMode === 'weekly') document.getElementById('btn-weekly').classList.add('active'); else if (todoMode === 'routine') document.getElementById('btn-routine').classList.add('active');
    
    if(todoMode === 'daily') {
        document.getElementById('todo-today-date').innerText = new Date().toLocaleDateString('fr-FR', {weekday: 'long', day: 'numeric', month: 'long'}); c.innerHTML = '<div class="weekly-container"></div>'; const wc = c.querySelector('.weekly-container');
        for (let h = 8; h <= 20; h++) {
        ...
        document.getElementById('todo-today-date').innerText = "Configuration de la Semaine Type ⚙️"; c.innerHTML = '<div class="weekly-container"></div>'; const wc = c.querySelector('.weekly-container'); const presidentialOrder = [1, 2, 3, 4, 5, 6, 0];
        presidentialOrder.forEach(dayNum => {
...
// --- LOGIQUE DES ACCIONS DU LIEN DE PARTAGE ---
...
if(btnAddFriend) {
    btnAddFriend.onclick = () => {
        ...
        db.collection("users").where("shareCode", "==", code).get().then(snapshot => {
            ...
            if(!sharedWith.includes(currentUser.uid)) { sharedWith.push(currentUser.uid); db.collection("users").doc(friendUid).update({sharedWith: sharedWith}); }
            
            startFriendSync(friendUid, friendName); renderFriendsList();
            document.getElementById('friend-code-input').value = ""; showToast(`Agenda de ${friendName} lié ! ✨`);
        });
    };
}

document.getElementById('save-task').onclick = () => {
    const n = document.getElementById('task-name').value.trim(), dStr = document.getElementById('task-desc').value.trim(); 
    const singleDate = document.getElementById('task-date').value, time = getCustomTime('task-time'), imp = document.getElementById('task-importance').value, reminders = getSelectedRemindersFromBadges(); 
    if(n && currentUser) {
        if(document.getElementById('modal-title').innerText === "Dupliquer la tâche" && editingId) {
            const task = tasks.find(t => t.id === editingId);
            if (task && singleDate) {
                let allOccurrences = [{date: task.date, time: task.time || ""}], currentGhosts = Array.isArray(task.duplicateDays) ? task.duplicateDays : [];
            ...
            editingId = null; showToast("Tâche modifiée ! ✎"); 
        } else if (singleDate) {
...
document.getElementById('save-todo').onclick = () => {
    const n = document.getElementById('todo-task-name').value.trim(), t = getCustomTime('todo-time');
    const isWeekly = document.getElementById('save-todo').getAttribute('data-weekly-mode') === 'true', isRoutine = document.getElementById('save-todo').getAttribute('data-routine-mode') === 'true';
    if(n && t && currentUser) { 
...
if (btnSaveBirthday) {
    btnSaveBirthday.onclick = () => {
        ...
        if (n && d && currentUser) { db.collection("birthdays").add({ name: n, date: d, userId: currentUser.uid, createdAt: Date.now() }).then(() => { showToast("Anniversaire enregistré ! 🎂"); document.getElementById('birthday-modal').style.display = 'none'; }); }
    };
}
...
document.getElementById('btn-login').onclick = () => { const email = document.getElementById('auth-email').value, pass = document.getElementById('auth-pass').value; if(email && pass) auth.signInWithEmailAndPassword(email, pass).then(() => { showToast("Ravi de vous revoir ! 👋"); }).catch(err => showToast("Erreur : " + err.message)); };
document.getElementById('btn-register').onclick = () => { const email = document.getElementById('auth-email').value, pass = document.getElementById('auth-pass').value; if(email && pass) auth.createUserWithEmailAndPassword(email, pass).then(() => showToast("Compte créé avec succès ! 🎉")).catch(err => showToast("Erreur : " + err.message)); };
...
document.getElementById('btn-save-nickname').onclick = () => { const nick = document.getElementById('profile-nickname').value.trim(); if (nick && currentUser) { db.collection("users").doc(currentUser.uid).set({ nickname: nick }, { merge: true }).then(() => { userNickname = nick; showToast("Surnom enregistré avec succès ! ✨"); }); } };

window.onclick = (e) => { 
    if(e.target.classList && e.target.classList.contains('modal')) { 
        unlockModalFields(); document.getElementById('task-modal').style.display = 'none'; document.getElementById('todo-modal').style.display = 'none'; document.getElementById('calendar-day-modal').style.display = 'none'; document.getElementById('welcome-modal').style.display = 'none'; 
        if(document.getElementById('ghost-modal')) document.getElementById('ghost-modal').style.display = 'none'; if(document.getElementById('birthday-modal')) document.getElementById('birthday-modal').style.display = 'none';
    } 
};
