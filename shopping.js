...
function renderShoppingCategories() {
    const container = document.getElementById('shopping-categories'); const breadcrumb = document.getElementById('shopping-breadcrumb'); if(!container || !breadcrumb) return;

    // --- SÉCURITÉ : VÉRIFICATION ACCÈS RESTREINT ÉVÉNEMENT POUR LES INVITÉS ---
        ...
        if (listObj && listObj.type === 'event' && listObj.createdBy !== currentUser.uid) {
            ...
            container.innerHTML = `
                <div style="grid-column: 1 / -1; background: rgba(128,128,128,0.05); border: 2px dashed var(--warning); padding: 25px 15px; border-radius: 12px; text-align: center;">
                    ...
                    <span style="font-style: italic; opacity: 0.8; font-size: 0.9rem;">Vous êtes invité à cet événement. Seul l'organisateur gère la liste et ajoute les produits.</span>
                </div>`;
            return; 
        }
    }
    ...
    if (currentShoppingPath.length === 0) { breadcrumb.innerText = ''; } else { breadcrumb.innerText = currentShoppingPath[currentShoppingPath.length - 1]; }
    const isAtRoot = (currentShoppingPath.length === 0), currentPathStr = currentShoppingPath.join('/');

    let navRowHtml = `<div style="grid-column: 1 / -1; display: flex; gap: 10px; width: 100%; align-items: center; margin-bottom: 5px;"><div onclick="${isAtRoot ? '' : 'shoppingNavigateBack()'}" style="flex: 1; background:rgba(128,128,128,0.1); color:var(--text-color); padding:12px; border-radius:12px; text-align:center; font-weight:bold; ${isAtRoot ? 'opacity: 0.3; cursor: default; pointer-events: none;' : 'cursor: pointer;'}">⬅️ Retour</div><input type="text" id="shopping-search" placeholder="🔍 Rechercher..." value="${shoppingSearchQuery}" oninput="handleShoppingSearch(this.value)" style="flex: 2; padding: 12px; border-radius: 12px; border: 1px solid rgba(128,128,128,0.3); background: var(--card-bg); color: var(--text-color); font-size: 1rem; font-weight: bold; outline: none;"></div>`;
    ...
    if (shoppingSearchQuery.trim() !== "") {
        let matches = []; let q = shoppingSearchQuery.toLowerCase().trim();
        ...
        matches.forEach(product => {
            let safe = product.isCustom ? product.id.replace(/'/g, "\\'") : product.name.replace(/'/g, "\\'");
            container.innerHTML += `<div onclick="openShoppingItemModal('${safe}', ${product.isCustom})" style="background:var(--card-bg); padding:15px; border-radius:12px; text-align:center; box-shadow:0 4px 6px rgba(0,0,0,0.05); font-weight:bold; cursor:pointer; border:1px solid rgba(128,128,128,0.2); transition: transform 0.2s;">+ ${product.isCustom ? product.name : formatProductDisplay(product.name)}</div>`;
        });
    } else {
        let defaultFolders = [], defaultProducts = []; let currentObj = foodCategories;
