...
function openCustomShoppingListShareModal() {
    ...
    // On force le rendu immédiatement avec les données déjà en mémoire
    console.log("Ouverture modale. Listes actuelles :", mySharedLists);
...
function renderMySharedListsInModal() {
    ...
    // Si mySharedLists est vide, on logue pour voir si le problème vient du chargement
    if (!mySharedLists || mySharedLists.length === 0) {
        container.innerHTML = `<p style="font-size: 0.85rem; opacity: 0.5; font-style: italic; text-align: center; padding: 10px;">Aucune liste partagée active trouvée.</p>`;
        console.log("renderMySharedListsInModal : mySharedLists est vide.");
        return;
    }
